...
def check_movimiento(pieza_select, mov_x, mov_y, closest_x_up, closest_y_up, closest_x_down, closest_y_down):
    ...
    return closest_x_down, closest_y_down                
...
for a in range(8):
    for b in range(8):
        ...
        tablero_grid_2.update({f"{tablero_y[b]}{tablero_x[a]}":(pos_x, pos_y)})
        ...
        if event.type == pygame.QUIT:
            ...
            if turno_w:
                ...
                for i in tablero_grid:
                    cords = i
                    grid = tablero_grid[i]
                    pieza = piezas_positions[grid]
                    if pieza.endswith("b"):
                        piezas_bot_cords.append(cords)
                        piezas_bot_grid.append(grid)
                        piezas_bot.append(pieza)
                
                index_pieza = random.randint(0,len(piezas_bot)-1)
                print(piezas_bot[index_pieza])

        
                x_up_random = piezas_bot_cords[index_pieza][0]
                y_up_random = piezas_bot_cords[index_pieza][1]
                closest_x_up = closest_value(x_up_random, tablero_cords_x)
                closest_y_up = closest_value(y_up_random, tablero_cords_y)
            ...
            if turno_w:
                ...
                x_down_random = x_up_random
                y_down_random = y_up_random
                
                closest_x_down = closest_value(x_down_random, tablero_cords_x)
                closest_y_down = closest_value(y_down_random, tablero_cords_y)
                print(closest_x_down, closest_y_down)
            ...
            #Movimiento ficha
            closest_x_down, closest_y_down = check_movimiento(pieza_select, mov_x, mov_y, closest_x_up, closest_y_up, closest_x_down, closest_y_down)
            ...
            #Comprobar por ficha comida despues de haber comprobado turnos
            ficha_comida = piezas_positions[tablero_grid[(closest_x_down, closest_y_down)]]
